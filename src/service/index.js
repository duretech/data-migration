...
class DataService {
  ...
  getIndicatorData(
    ...
    if (ngoValue && ageGroupValue) {
      ...
      if (msiGroupValue)
        url =
          `${urlNew}/api/analytics.json?dimension=dx:` +
          indID +
          `&dimension=` +
          ngoValue +
          `&dimension=` +
  ...
  getDataElementsByDataSets(DSID, secondary, signal) {
    ...
    return axios({
      method: "get",
      url: `${url}/api/dataElements.json?filter=dataSetElements.dataSet.id:eq:${DSID}&fields=description,id,displayName&paging=false`,
      // url: `${store.getters.getBaseURL}/api/dataElements.json?fields=id,displayName,categoryCombo%5bid,categoryOptionCombos%5bid%5d%5d&paging=false`,
      // url: `${store.getters.getBaseURL}/api/dataElements.json?fields=id,displayName,formName,description,categoryCombo[id,displayName,categoryOptionCombos[id,displayName]]&paging=false`,
      headers: secondary ? store.getters.getSecondaryHeader : header,
      signal,
  ...
  getDataSets({ secondary }) {
    ...
    return axios({
      method: "get",
      // url: `${store.getters.getBaseURL}/api/programIndicators.json`,
      url: `${url}/api/dataSets.json?paging=false&fields=displayName,id,periodType`,
      headers: secondary ? store.getters.getSecondaryHeader : header,
    });
  }
  /**
  ...
  getOrganisation({ secondary }) {
    ...
    return axios({
      method: "get",
      url: `${url}/api/organisationUnits.json?fields=displayName,id,name,level,parent[id, name]&paging=false`,
      headers: secondary ? store.getters.getSecondaryHeader : header,
    });
  }
  /**
   ...
   * @returns response object
   */
  getOrganisationUnitLevels({ secondary }) {
    let url = secondary
      ? store.getters.getSecondaryServerUrl
      : store.getters.getBaseURL;
    ...
    return axios({
      method: "get",
      url: `${url}/api/filledOrganisationUnitLevels`,
      headers: secondary ? store.getters.getSecondaryHeader : header,
    });
  }
  /**
   ...
   * @returns response object
   */
  getOrganisationChildren(uid, level, secondary, signal) {
    let urlNew = secondary
      ? store.getters.getSecondaryServerUrl
      : store.getters.getBaseURL;

    //let orgLevel = 8;
    let url = `${urlNew}/api/organisationUnits/${uid}.json?fields=id,displayName,level,parent[id, name],children[id,displayName,level,parent[id, name]]`;
   ...
   * @returns response object
   */
  renameKeys(obj, addLevelID = true, selectedOrgLevel = null) {
    ...
    if (!findChildren.includes("isDisabled")) {
      // console.log("findChildren", findChildren.includes("children"));
    ...
    const keyValues = Object.keys(obj)
      .map((key) => {
        ...
        // console.log(obj, "in level if", selectedOrgLevel);
        if (key === "level" && selectedOrgLevel) {
          if (obj[key] > selectedOrgLevel) obj["isDisabled"] = true;
        }
        if (key === "children") {
          obj[key] =
            obj[key]
              ?.map((obj) => this.renameKeys(obj, addLevelID, selectedOrgLevel))
              .sort((a, b) =>
